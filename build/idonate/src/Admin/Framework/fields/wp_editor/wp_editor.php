 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'IDONATE_Field_wp_editor' ) ) {
	class IDONATE_Field_wp_editor extends IDONATE_Fields {
		...
		public function render() {
			...
			echo ( idonate_wp_editor_api() ) ? '<div class="idonate-wp-editor" data-editor-settings="' . esc_attr( wp_json_encode( $editor_settings ) ) . '">' : '';
			...
			echo ( idonate_wp_editor_api() ) ? '</div>' : '';
		...
		public function enqueue() {
			...
			if ( idonate_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
		...
		public function setup_wp_editor_media_buttons() {
			...
			echo 'var idonate_media_buttons = ' . wp_json_encode( $media_buttons ) . ';';
			echo '</script>';
		}
		...
		public function setup_wp_editor_settings() {
			...
			if ( idonate_wp_editor_api() && class_exists( '_WP_Editors' ) ) {
				...
				$setup = _WP_Editors::parse_settings( 'idonate_wp_editor', $defaults );

				_WP_Editors::editor_settings( 'idonate_wp_editor', $setup );
