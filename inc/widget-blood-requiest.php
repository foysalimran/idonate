...
esc_html__( 'Blood Request Widget', 'idonate' ), 
	...
	<div class="blood-request-widget">
		...
		if( $loop->have_posts() ){
			...
			while( $loop->have_posts() ){
				...
				//
				if( $title ){
					$title = '<span class="display-block">'.esc_html( $title ).'</span>';
				}else{
					$title = '';
				}
				//
				if( $bloodgroup ){
					$bloodgroup = '<span class="mr-5">'.esc_html__( 'Blood Group: ', 'idonate' ).esc_html( $bloodgroup ).'</span>';
				}else{
					$bloodgroup = '';
				}
				//
				if( $date ){
					$date = '<span class="ml-5">'.esc_html__( 'Date: ', 'idonate' ).esc_html( $date ).'</span>';
				}else{
					$date = '';
				}
				//
...
<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'requestnumber' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'requestnumber' ) ); ?>" type="text" value="<?php echo esc_html( $requestnumber ); ?>" />
</p>
