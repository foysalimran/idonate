 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'IDONATE_Field_dimensions' ) ) {
	class IDONATE_Field_dimensions extends IDONATE_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['width'] ) ) {
				...
				echo ( ! empty( $args['width_icon'] ) ) ? '<span class="idonate--label idonate--icon">' . wp_kses_post($args['width_icon']) . '</span>' : '';
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[width]' ) ) . '" value="' . esc_attr( $value['width'] ) . '"' . wp_kses_post($placeholder) . ' class="idonate-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				echo ( ! empty( $unit ) ) ? '<span class="idonate--label idonate--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
			}
			...
			if ( ! empty( $args['height'] ) ) {
				...
				echo '<div class="idonate--input">';
				echo ( ! empty( $args['height_icon'] ) ) ? '<span class="idonate--label idonate--icon">' . wp_kses_post($args['height_icon']) . '</span>' : '';
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[height]' ) ) . '" value="' . esc_attr( $value['height'] ) . '"' . wp_kses_post($placeholder) . ' class="idonate-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				echo ( ! empty( $unit ) ) ? '<span class="idonate--label idonate--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
			}
