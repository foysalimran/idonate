 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'IDONATE_Field_link_color' ) ) {
	class IDONATE_Field_link_color extends IDONATE_Fields {
		...
		public function render() {
			...
			$color_props = array(
				'color'   => esc_html__( 'Normal', 'idonate' ),
				'hover'   => esc_html__( 'Hover', 'idonate' ),
				'active'  => esc_html__( 'Active', 'idonate' ),
				'visited' => esc_html__( 'Visited', 'idonate' ),
				'focus'   => esc_html__( 'Focus', 'idonate' ),
			);
			...
			foreach ( $color_props as $color_prop_key => $color_prop_value ) {
				...
				if ( ! empty( $args[ $color_prop_key ] ) ) {
					...
					echo '<div class="idonate--title">' . esc_attr( $color_prop_value ) . '</div>';
					echo '<input type="text" name="' . esc_attr( $this->field_name( '[' . $color_prop_key . ']' ) ) . '" value="' . esc_attr( $value[ $color_prop_key ] ) . '" class="idonate-color"' . wp_kses_post($default_attr) . wp_kses_post( $this->field_attributes() ) . '/>';
					echo '</div>';
