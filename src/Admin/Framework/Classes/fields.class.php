 ...
 *
 */
if (!class_exists('IDONATE_Fields')) {
  /**
   *
   * Fields Class
   *
  ...
  abstract class IDONATE_Fields extends IDONATE_Abstract
  {
    ...
    {
      return (!empty($this->field['before'])) ? '<div class="idonate-before-text">' . $this->field['before'] . '</div>' : '';
    }
    ...
    public function field_after()
    {
      ...
      $output  = (!empty($this->field['after'])) ? '<div class="idonate-after-text">' . $this->field['after'] . '</div>' : '';
      $output .= (!empty($this->field['desc'])) ? '<div class="clear"></div><div class="idonate-desc-text">' . $this->field['desc'] . '</div>' : '';
      $output .= (!empty($this->field['help'])) ? '<div class="idonate-help"><span class="idonate-help-text">' . $this->field['help'] . '</span><i class="icofont-question-circle"></i></div>' : '';
      $output .= (!empty($this->field['_error'])) ? '<div class="idonate-error-text">' . $this->field['_error'] . '</div>' : '';
    ...
    public static function field_data($type = '', $term = false, $query_args = array(), $field_unique = null)
    {
      ...
      switch ($type) {
        ...
        case 'taxonomy':
          global $post;
          $view_options       = get_post_meta($post->ID, 'ta_idonate_view_options', true);
          $idonate_post_types     = 'post';
          $taxonomy_names = get_object_taxonomies($idonate_post_types, 'names');
          if (!is_wp_error($taxonomy_names) && !empty($taxonomy_names)) {
            $options[''] = esc_html__('Select Taxonomy', 'idonate');
        ...
        case 'term':
          ...
          $idonate_taxonomy = isset($view_options['idonate_filter_by_taxonomy']['idonate_taxonomy_and_terms'][$field_index]['idonate_select_taxonomy']) ? $view_options['idonate_filter_by_taxonomy']['idonate_taxonomy_and_terms'][$field_index]['idonate_select_taxonomy'] : get_object_taxonomies($idonate_post_types, 'names');
          if (version_compare(get_bloginfo('version'), '4.5', '>=')) {
            $terms = get_terms(array('taxonomy' => $idonate_taxonomy));
          } else {
            $terms = get_terms(array($idonate_taxonomy));
          }
