 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'IDONATE_Field_spacing' ) ) {
	class IDONATE_Field_spacing extends IDONATE_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[all]' ) ) . '" value="' . esc_attr( $value['all'] ) . '"' . wp_kses_post($placeholder) . ' class="idonate-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				echo ( $unit ) ? '<span class="idonate--label idonate--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . wp_kses_post($placeholder) . ' class="idonate-input-number' . esc_attr( $is_unit ) . '" step="any" />';
					echo ( $unit ) ? '<span class="idonate--label idonate--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
